...
constant regs_mcore = {"r0","r1","r2","r3","r4","r5","r6","r7",
...
--function get_reg2_mcore(sequence s)
--	integer p,q
--	
--	p = find(',',s)
--	q = find(')',s)
...
--function get_reg1_mcore(sequence s)
--	integer p
--	
--	p = find(',',s)
--	if p then
...
function compare_patterns_mcore(sequence s1,sequence s2)
...
---- New pattern matcher
			...
			while p1 <= length(s1) do
				...
				--	puts(1,"Trying to match an streg\n")
					...
					o = 0
					for i=1 to length(regs_mcore) do
						if equal(s4,regs_mcore[i]) then
							eql = 1
							exit
						-- Are there any strings left that could
						-- potentially match if we keep reading
						-- more characters?
						elsif p4 < length(regs_mcore[i]) then
							o = 1
						end if
					end for
...
function pattern_append_mcore(sequence s1,sequence s2)
...
function pattern_optimise_mcore(sequence subject,sequence patterns,integer maxIterations)
					...
					for j=1 to length(pat[1]) do
						...
						if sequence(pat[1][j][1]) then
							...
							for k=1 to length(pat[1][j]) do
								if compare_patterns_mcore(subject[p+j-1],pat[1][j][k]) then
									m = k
									exit
								end if
							end for
							...
							if not compare_patterns_mcore(subject[p+j-1],pat[1][j]) then
								n = 0
								exit
							end if
						end if
							...
							for j=1 to length(pat[2]) do
								if sequence(pat[2][j]) then
									if sequence(pat[2][j][1]) then
										s = pattern_append_mcore(s,pat[2][j][1])
										newlen += 1
									else
										if pat[2][j][1] = COND then
											if pat[2][j][2] = NUMRANGE then
												...
												if t[2]>=pat[2][j][4] and t[2]<=pat[2][j][5] and pat[2][j][6] then
													s = pattern_append_mcore(s,pat[2][j][7])
													newlen += 1
											...
											s = pattern_append_mcore(s,subject[p+pat[2][j][1]-1][pat[2][j][2]..pat[2][j][3]]&pat[2][j][4])
											newlen += 1
										end if
									end if
								else
									s = pattern_append_mcore(s,subject[p+pat[2][j]-1])
									newlen += 1
								end if
							end for
...
function optimise_return_stack_mcore(sequence subject)
	integer m,n,p,q
	integer clean
	
	p = 1
	while p<length(subject) do
		if equal(subject[p],"subi r11,4") then
			m = p
			q = p
			clean = 1
			while q<=length(subject) do
				if equal(subject[q],"addi r11,4") then
					exit
				elsif compare_patterns_mcore(subject[q],"bsr /label/") then
					clean = 0
					exit
				end if
				q += 1
			...
			if clean then
				subject = subject[1..m-1]&subject[m+2..q-2]&subject[q+1..length(subject)]
				p = q-4
				linesRemoved += 4
			end if
		end if
...
function optimise_constant_assignments_mcore(sequence subject)
	integer l,m,n,o,p,q
				   ...
				   (equal(subject[p][1..5],"loop ")) then
					...
					if (length(subject[p])=21 and equal(subject[p][17..20],subject[m][13..16])) or
					   equal(subject[p][8..11],subject[m][8..11]) then
						...
						r = repeat(-1,length(regs_mcore)) 	-- Unused / constant / variable
						...
						while m<n do
							if length(subject[m])>6 then
								...
								--	s = {0,find(subject[m][length(subject[m])-1..length(subject[m])],regs68k)}
									patvars = {}
									if compare_patterns_mcore(subject[m],"/reg/ = /imm//ext/;") and
									   r[find(patvars[1],regs_mcore)] != -1 then
									   	l = find(patvars[1],regs_mcore)
									...
									--patvars = {}
									--if compare_patterns_mcore(subject[m],"move.l #/const/,/reg/") and r[s[2]]!=-1 then
									--	if r[s[2]]=0 then
									--		t[s[2]] = patvars[1]
									--		u[s[2]] = {m}
									--		r[s[2]] = 1
								...
								--end if
								patvars = {}
								if compare_patterns_mcore(subject[m],"call /label/;") then
									r = repeat(-1,length(r))
									exit
								end if
							end if
						...
						for i=1 to length(r) do
							if r[i]=1 then
								...
								subject = subject[1..q-1] & {sprintf(regs_mcore[i]&" = ",i-1)&t[i]} & subject[q..length(subject)]
								q += 1
								u += 1
								improvement = 1
...
function optimise_register_usage_mcore(sequence subject)
	integer m,n,o,p,q,d
				...
				if equal(subject[p][1..7],"__loop_") and not find('e',subject[p]) then
					...
					if equal(subject[p][8..11],subject[loopStart][8..11]) then
						...
						while m<loopEnd do
							...
							patvars = {}
							depth &= d
							if compare_patterns_mcore(subject[m],"p1 += /imm/;") then
								s = value(patvars[1])
								if s[2]=-4 then
									d -= s[2]
									if d > maxDepth then
							...
							elsif compare_patterns_mcore(subject[m],"call /label/;") then
								clean = 0
								exit
							elsif compare_patterns_mcore(subject[m],"/reg/ = [p1++];") then
								d -= 4
								r[find(patvars[length(patvars)],regs_mcore)] = 1
							elsif compare_patterns_mcore(subject[m],"/reg/ = /reg/;") then
								r[find(patvars[length(patvars)-1],regs_mcore)] = 1
							elsif compare_patterns_mcore(subject[m],"/reg/ = /imm//ext/;") then
								r[find(patvars[length(patvars)-2],regs_mcore)] = 1
							end if
						...
						if clean and d=0 and find(0,r)>0 then
							...
							while m<loopEnd do
								...
								if compare_patterns_mcore(subject[m],"p1 += /imm/;") then
									...
									--end if
									--if s[2] = -4 then
									--	if compare_patterns_mcore(subject[m+1],"[p1];")
								...
								elsif compare_patterns_mcore(subject[m],"[p1] = /reg/;") then
									if depth[n]>1 and depth[n]<=length(r)+1 then
										subject[m] = regs_mcore[r[depth[n]-1]]&" = "&patvars[length(patvars)]&";"
									end if
								elsif compare_patterns_mcore(subject[m],"/reg/ = [p1];") then
									if depth[n]>1 and depth[n]<=length(r)+1 then
										subject[m] = patvars[length(patvars)]&" = "&regs_mcore[r[depth[n]-1]]&";"
									end if
								
								elsif compare_patterns_mcore(subject[m],"/reg/ = [p1 + /imm/];") then
								elsif compare_patterns_mcore(subject[m],"[p1 + /imm/] = /reg/;") then
								elsif compare_patterns_mcore(subject[m],"/reg/ = [p1++];") then
									if depth[n]>1 and depth[n]<=length(r)+1 then
										subject[m] = patvars[length(patvars)]&" = "&regs_mcore[r[depth[n]-1]]&";"
									end if
...
global function optimise_mcore(sequence subject,integer remConst)
	...
	if optLevel >= 5 and 0 then
		if remConst then
			...
			for i=1 to length(subject) do
				patvars = {}
				--if compare_patterns_mcore(subject[i],"ldr r1,/const/") then
	...
	if optLevel >= 5 then
		subject = pattern_optimise_mcore(subject,intpatterns_mcore,10)

		subject = optimise_return_stack_mcore(subject)

		--subject = optimise_register_usage_mcore(subject)
		
		--subject = optimise_constant_assignments_mcore(subject)
		
		--subject = pattern_optimise_mcore(subject,intpatterns2_mcore,4)
	end if
